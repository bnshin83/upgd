...
[Moosavi-Dezfooli et al., 2019, Fawzi et al., 2018], coresets [Garg and Roy, 2023] and memorization [Garg et al., 2024, Ravikumar et al., 2024], mainly focusing on its properties on the train set. Thus, there is a gap in our understanding of input loss curvature on unseen test examples.
...
Additionally, the concept of adjacent (or neighboring) datasets is linked to the distance between datasets. The distance between two datasets $S$ and $S^{\prime}$, denoted by $\left\|S-S^{\prime}\right\|_{1}$, measures the number of samples that differ between them. The notation $\|S\|_{1}$ represents the size of a dataset $S$. When a randomized learning algorithm $\mathcal{A}$ is applied to a dataset $S$, it produces a hypothesis denoted by $h_{S}^{\phi}=\mathcal{A}(\phi, S)$, where $\phi \sim \Phi$ is the random variable associated with the algorithm's randomness. A cost function $c: \mathcal{Y} \times \mathcal{Y} \rightarrow \mathbb{R}^{+}$is used to measure the hypothesis's performance. The cost of the hypothesis $h$ at a sample $z_{i}$ is also referred to as the loss $\ell$ at $z_{i}$, defined as $\ell\left(h, z_{i}\right)=c\left(h\left(x_{i}\right), y_{i}\right)$. The performance of a hypothesis $h$ is measured using risk $R(h)=\mathbb{E}_{z_{i}}\left[\ell\left(h, z_{i}\right)\right]$ and approximated using empirical risk $R_{\text {emp }}(h)=(1 / m) \sum_{i=1}^{m} \ell\left(h, z_{i}\right)$.
Differential Privacy. Introduced by Dwork et al. [2006], differential privacy is defined as follows: A randomized algorithm $\mathcal{A}$ with domain $\mathcal{Z}^{m}$ is considered $\epsilon$-differentially private if for any subset $\mathcal{R} \subset \operatorname{Range}(\mathcal{A})$ and for any datasets $S, S^{\prime} \in \mathcal{Z}^{m}$ differing in at most one element (i.e. $\left\|S-S^{\prime}\right\|_{1} \leq 1$ ):
...
$$
\begin{equation*}
\forall i \in\{1, \cdots, m\},\left|\mathbb{E}_{\phi, z}\left[\ell\left(h_{S}^{\phi}, z\right)\right]-\mathbb{E}_{\phi, z}\left[\ell\left(h_{S \backslash i}^{\phi}, z\right)\right]\right| \leq \beta, \tag{2}
\end{equation*}
$$
...
$$
\begin{equation*}
\operatorname{Pr}\left[\left|R_{e m p}(h, S)-R(h)\right| \leq \gamma^{\prime}(m)\right] \geq \delta . \tag{3}
\end{equation*}
$$

Uniform Model Bias. The hypothesis $h$ produced by algorithm $\mathcal{A}$ to learn the true conditional $h^{*}=\mathbb{E}[y \mid x]$ from a dataset $S \sim \mathcal{D}^{m}$ has uniform bound on model bias denoted by $\Delta$ if:

$$
\begin{equation*}
\forall S \sim \mathcal{D}^{m}, \quad\left|\mathbb{E}_{\phi}\left[R\left(h_{S}^{\phi}\right)-R\left(h^{*}\right)\right]\right| \leq \Delta . \tag{4}
\end{equation*}
$$

$\rho$-Lipschitz Hessian. The Hessian of $\ell$ is Lipschitz continuous on $\mathcal{Z}$, if $\forall z_{1}, z_{2} \in \mathcal{Z}$, and $\forall h \in \operatorname{Range}(\mathcal{A})$, there exists some $\rho>0$ such that:

$$
\begin{equation*}
\left\|\nabla_{z_{1}}^{2} \ell\left(h, z_{1}\right)-\nabla_{z_{2}}^{2} \ell\left(h, z_{2}\right)\right\| \leq \rho\left\|z_{1}-z_{2}\right\| . \tag{5}
\end{equation*}
$$
...
$$
\begin{equation*}
\operatorname{Curv}_{\phi}\left(z_{i}, S\right)=\operatorname{tr}(H)=\operatorname{tr}\left(\nabla_{z_{i}}^{2} \ell\left(h_{S}^{\phi}, z_{i}\right)\right) \tag{6}
\end{equation*}
$$

$v$-adjacency. A dataset $S$ is said to contain $v$-adjacent (read as upsilon-adjacent) elements if it contains two elements $z_{i}, z_{j}$ such that $z_{j}=z_{i}+\alpha$ for some $\alpha \in B_{p}(v)$ (read as $v$-Ball). This condition can be ensured through construction. Consider a dataset $S^{\prime}$ which has no $z_{j}$ s.t $z_{j}=z_{i}+\alpha ; z_{j}, z_{i} \in S^{\prime}$. We can construct $S$ such that $S=\left\{z \mid z \in S^{\prime}\right\} \cup\left\{z_{i}+\alpha\right\}$ for some $z_{i} \in S^{\prime}, \alpha \in B_{p}(v)$, thereby ensuring $v$-adjacency.
...
![](https://cdn.mathpix.com/cropped/2025_11_08_81705b7b7843ea88add3g-05.jpg?height=526&width=662&top_left_y=252&top_left_x=374)
Figure 2: Marginal curvature score histogram for all images in ImageNet when samples in train set vs test set.

![](https://cdn.mathpix.com/cropped/2025_11_08_81705b7b7843ea88add3g-05.jpg?height=529&width=643&top_left_y=249&top_left_x=1084)
Figure 3: Conditional curvature scores for a single image from ImageNet when using 56 shadow models.
...
Setup: We use CIFAR10, CIFAR100 and ImageNet datasets to test the MIA performance. We consider a black-box MIA setup similar to Carlini et al. [2022]. We use ResNet18 for CIFAR10 and CIFAR100, for ImageNet we use the ResNet50 architecture. For all the membership inference attacks, we compute a full ROC curve and report the results. When using shadow models we 64 for CIFAR10 and CIFAR100 and 52 for ImageNet. The AUROC plot for CIFAR100 for various

| Method |  | ImageNet |  | CIFAR100 |  | CIFAR10 |  |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
|  |  | Bal. Acc. | AUROC | Bal. Acc. | AUROC | Bal. Acc. | AUROC |
| Curv ZO NLL (Ours) |  | $\mathbf{6 9 . 1 6} \pm \mathbf{0 . 0 8}$ | $\mathbf{7 7 . 4 5} \pm \mathbf{0 . 0 9}$ | $\mathbf{8 4 . 4 7} \pm \mathbf{0 . 2 1}$ | $\mathbf{9 3 . 4 9} \pm \mathbf{0 . 1 8}$ | $\mathbf{6 1 . 9 2} \pm \mathbf{0 . 8 7}$ | $\mathbf{6 8 . 8 2} \pm \mathbf{1 . 3 0}$ |
| Curv ZO LR (Ours) |  | $68.76 \pm 0.04$ | $72.28 \pm 0.04$ | $80.48 \pm 0.10$ | $90.15 \pm 0.04$ | $55.00 \pm 0.17$ | $58.89 \pm 0.38$ |
| Carlini et al. \|2022 |  | $66.14 \pm 0.01$ | $73.46 \pm 0.02$ | $81.55 \pm 0.13$ | $88.89 \pm 0.16$ | $58.23 \pm 0.29$ | $61.73 \pm 0.32$ |
| Yeom et al. \|2018 |  | $58.50 \pm 0.02$ | $63.23 \pm 0.03$ | $76.29 \pm 0.39$ | $82.11 \pm 0.31$ | $55.57 \pm 0.52$ | $60.44 \pm 0.75$ |
| Sablayrolles et al. 2019 |  | $66.93 \pm 0.05$ | $76.50 \pm 0.04$ | $70.22 \pm 0.41$ | $81.11 \pm 0.39$ | $56.65 \pm 0.56$ | $61.50 \pm 0.79$ |
| Watson et al. \|2022 |  | $61.40 \pm 0.06$ | $69.44 \pm 0.05$ | $62.71 \pm 0.31$ | $71.66 \pm 0.50$ | $54.86 \pm 0.59$ | $58.58 \pm 0.86$ |
| Ye et al. \|2022 |  | $66.16 \pm 0.02$ | $75.79 \pm 0.05$ | $80.73 \pm 0.24$ | $90.88 \pm 0.19$ | $59.62 \pm 0.84$ | $67.30 \pm 1.25$ |
| Song and Mittal | 2021 | $57.88 \pm 0.03$ | $63.29 \pm 0.03$ | $75.58 \pm 0.29$ | $82.28 \pm 0.27$ | $55.63 \pm 0.61$ | $60.42 \pm 0.85$ |

Table 1: Comparison of the proposed curvature score based MIA with prior methods tested on ImageNet, CIFAR100, and CIFAR10 datasets. Results reported are the mean $\pm$ std obtained over 3 seeds. For CIFAR10 and CIFAR100 64 shadow models were used and 52 for ImagNet.

methods are shown in Figure 4 . Table 1 reports the average balanced accuracy and AUROC values over three seeds on various MIA methods on the three datasets. The plot in Figure 4 is a log-log plot to emphasize performance of the proposed method at very low false positive rates (see the orange line y-intercept). We also studied the effect of augmentations and the results can be found in Appendix A.7 the takeaway was that adding more augmentations improved performance. Note that the results presented in Table 1 used 2 augmentations (image + mirror) for our technique and Carlini et al. [2022] for fair comparison.
...
![](https://cdn.mathpix.com/cropped/2025_11_08_81705b7b7843ea88add3g-08.jpg?height=484&width=654&top_left_y=1864&top_left_x=379)
Figure 4: Comparing our method against existing techniques at low FPR. The proposed parametric Curv LR technique has the highest TPR at very low FPR.

![](https://cdn.mathpix.com/cropped/2025_11_08_81705b7b7843ea88add3g-08.jpg?height=483&width=654&top_left_y=1870&top_left_x=1089)
...
![](https://cdn.mathpix.com/cropped/2025_11_08_81705b7b7843ea88add3g-09.jpg?height=524&width=665&top_left_y=249&top_left_x=371)
Figure 6: Visualizing MIA performance as a function of the size of the train set, which is randomly sampled.

![](https://cdn.mathpix.com/cropped/2025_11_08_81705b7b7843ea88add3g-09.jpg?height=521&width=676&top_left_y=252&top_left_x=1073)
Figure 7: MIA performance as a function of the size of the train set when subsets contain the lowest curvature examples from the entire set.

plot the result of the proposed curvature based MIA in Figure 5 In Figure 5 we also plots the best fit using $s_{f}\left(L_{f}\left(1-e^{-\epsilon}\right)+c_{f}\right)^{2}$, where $s_{f}, L_{f}$ and $c_{f}$ are fit to the data.
...
I. Garg, D. Ravikumar, and K. Roy. Memorization through the lens of curvature of loss function around samples. In Forty-first International Conference on Machine Learning, 2024. URL https://openreview.net/forum?id=WQbDS9RydY,
...
D. Ravikumar, E. Soufleri, A. Hashemi, and K. Roy. Unveiling privacy, memorization, and input curvature links. In Forty-first International Conference on Machine Learning, 2024. URL https://openreview.net/forum?id=4dxR7aw05n.
...
L. Watson, C. Guo, G. Cormode, and A. Sablayrolles. On the importance of difficulty calibration in membership inference attacks. In International Conference on Learning Representations, 2022. URL https://openreview.net/forum?id=3eIrli0TwQ.
...
## A. 1 Zero Order Curvature Estimation
...
Algorithm 1 Pseudo-code for Zero Order Input Loss Curvature Estimation
    Input: $z_{i}=\left(x_{i}, y_{i}\right)$ (image, label data), $\ell$ (loss function), $n_{\text {iter }}$ (hyperparameter: number of
    iterations), $h$ (hyperparameter: small increment), $g$ (neural network)
    Output: curv_estimate (zero order input loss curvature estimate of $z_{i}$ )
    curv $\leftarrow 0$
    for $i \leftarrow 1$ to $n_{\text {iter }}$ do
        $v \sim$ Rademacher
        $u \sim$ Rademacher
        $f \leftarrow \ell\left(g\left(x_{i}\right), y_{i}\right)$
        $\nabla^{2} f\left(z_{i}\right) \leftarrow$ Use Equation 11
        curv $\leftarrow$ curv $+\operatorname{tr}\left(\nabla^{2} f\left(z_{i}\right)\right)$
    end for
    curv_estimate $\leftarrow$ curv $/ n_{\text {iter }}$
    return curv_estimate
```


## A. 2 Curvature based MIA
...
1. Initialization (Lines 3-4): Two empty sets, $c u r v_{i n}$ and $c u r v_{o u t}$, are initialized. These sets will store the curvature scores for the shadow models trained with and without the target example $z_{i}$.
2. Shadow Model Training (Lines 5-12): For a specified number of iterations $N$, the algorithm performs the following steps:

- Sampling Shadow Dataset (Line 6): A shadow dataset $\mathcal{D}_{\text {attack }}$ is sampled from the data distribution $\mathcal{D}$.
- Training IN Shadow Model (Line 7): A shadow model $g_{i n}^{n}$ is trained on the shadow dataset augmented with the target example, $\mathcal{D}_{\text {attack }} \cup\left\{z_{i}\right\}$. The curvature score for this model, denoted as $c u r v_{i n}^{n}$, is computed using Algorithm 1.
- Collecting IN Curvature Scores (Line 8): The curvature score for the training set $\mathcal{D}_{\text {attack }} \cup\left\{z_{i}\right\}$ of this model $\operatorname{curv}_{i n}^{n}$ is added to the set $\operatorname{curv}_{i n}$.
- Training OUT Shadow Model (Line 9): Another shadow model $g_{\text {out }}^{n}$ is trained on the shadow dataset excluding the target example, $\mathcal{D}_{\text {attack }} \backslash\left\{z_{i}\right\}$. The curvature score for this model, denoted as $c u r v_{\text {out }}^{n}$, is also computed using Algorithm 1.
- Collecting OUT Curvature Scores (Line 10): The curvature score for the training set $\mathcal{D}_{\text {attack }} \backslash\left\{z_{i}\right\}$ of this model curv $v_{\text {out }}^{n}$ is added to the set curvout .

3. Model Parameter Estimation (Lines 13-16): After training the shadow models, the algorithm calculates the mean ( $\mu_{\text {in }}, \mu_{\text {out }}$ ) and variance ( $\sigma_{\text {in }}^{2}, \sigma_{\text {out }}^{2}$ ) of the collected curvature scores for both the 'IN' and 'OUT' models.
4. Target Model Query (Line 17): The curvature score for the target model $g_{\text {target }}$ with respect to the target example $z_{i}$, denoted as $c u r v_{\text {target }}$, is computed using Algorithm 1 .
5. Likelihood Ratio Test (Line 18-19): The likelihood ratio $P_{\text {in }}$ is calculated. This ratio compares the probability of the observed curvature score under the 'IN' distribution against the 'OUT' distribution and is returned. For the 'NLL' version $P_{\text {in }}$ us given by:

$$
\begin{equation*}
P_{\text {in }}^{N L L}=\log \left[P\left(\text { curv }_{\text {target }} \mid \mathcal{N}\left(\mu_{\text {out }}, \sigma_{\text {out }}^{2}\right)\right)\right]-\log \left[P\left(\text { curv }_{\text {target }} \mid \mathcal{N}\left(\mu_{\text {in }} \sigma_{\text {in }}^{2}\right)\right)\right] \tag{12}
\end{equation*}
$$

```
Algorithm 2 Membership Inference Attack using Input Loss Curvature Scores
Require: Target model $g_{\text {target }}$, target example $z_{i}=\left(x_{i}, y_{i}\right)$, data distribution $\mathcal{D}$
    curv $_{\text {in }} \leftarrow\{ \}$
    curv $_{\text {out }} \leftarrow\{ \}$
    for $n$ in $N$ do
        $\mathcal{D}_{\text {attack }} \leftarrow \mathcal{D} \quad \triangleright$ Sample a shadow dataset
        $g_{\text {in }}^{n} \leftarrow$ Train on $\mathcal{D}_{\text {attack }} \cup\left\{z_{i}\right\} . \quad \triangleright$ Train in shadow model
        $\operatorname{curv}_{\text {in }}^{n} \leftarrow$ Use Algorithm 1
        curv $_{\text {in }} \leftarrow$ curv $_{\text {in }} \cup$ curv $_{\text {in }}$
        $g_{\text {out }}^{n} \leftarrow$ Train on $\mathcal{D}_{\text {attack }} \backslash\left\{z_{i}\right\} . \quad \triangleright$ Train out shadow model
        $\operatorname{curv}_{\text {out }}^{n} \leftarrow$ Use Algorithm 1
        $\operatorname{curv}_{\text {out }} \leftarrow \operatorname{curv}_{\text {out }} \cup \operatorname{curv}_{\text {out }}^{n}$
    end for
    $\mu_{\text {in }} \leftarrow \operatorname{mean}\left(\right.$ curv $\left._{\text {in }}\right)$
    $\mu_{\text {out }} \leftarrow \operatorname{mean}\left(\right.$ curv $\left._{\text {out }}\right)$
    $\sigma_{\text {in }}^{2} \leftarrow \operatorname{var}\left(\right.$ curv $\left._{\text {in }}\right)$
    $\sigma_{\text {out }}^{2} \leftarrow \operatorname{var}\left(\right.$ curv $\left._{\text {out }}\right)$
    curv $_{\text {target }} \leftarrow$ Use Algorithm 1 with $g_{\text {target }}$ and $z_{i}$.
    $P_{\text {in }}=\frac{P\left(\text { curv }_{\text {target }} \mid \mathcal{N}\left(\mu_{\text {in }}, \sigma_{\text {in }}^{2}\right)\right)}{P\left(\text { curv }_{\text {target }} \mid \mathcal{N}\left(\mu_{\text {out }}, \sigma_{\text {out }}^{2}\right)\right)}$
    return $P_{\text {in }}$
```
...
Consider an $\epsilon-\mathrm{DP}$ algorithm and $S, S^{\backslash i}$ such that $\left\|S-S^{\backslash i}\right\|=1$. Next let $\mathcal{R} \subset \operatorname{Range}(\mathcal{A})$ such that $\mathcal{R}=\left\{h \mid h\left(x_{i}\right)=y_{i}\right\}$. Since $\mathcal{A}$ is $\epsilon$-differentially private, then it follows from the definition of differential privacy in Equation 1 that

$$
\begin{align*}
\operatorname{Pr}_{\phi}\left[h_{S}^{\phi} \in \mathcal{R}\right] & \leq e^{\epsilon} \operatorname{Pr}_{\phi}\left[h_{S \backslash i}^{\phi} \in \mathcal{R}\right]  \tag{13}\\
\Longrightarrow p\left(\phi, S, z_{i}\right) & \leq e^{\epsilon} p\left(\phi, S^{\backslash i}, z_{i}\right) \tag{14}
\end{align*}
$$

Since $p\left(\phi, S, z_{i}\right)=\operatorname{Pr}_{\phi}\left[h_{S}^{\phi} \in \mathcal{R}\right]$ and denotes the probability density function of the neural network's output probability. Now, we use the definition of KL divergence

$$
\begin{aligned}
\mathrm{D}_{\mathrm{KL}}\left(p\left(\phi, S, z_{i}\right) \| p\left(\phi, S^{\backslash i}, z_{i}\right)\right) & =\mathbb{E}_{\phi}\left[\log \left(\frac{p\left(\phi, S, z_{i}\right)}{p\left(\phi, S \backslash i, z_{i}\right)}\right)\right] \\
& \leq \mathbb{E}_{\phi}\left[\log e^{\epsilon}\right] \quad \text { From Equation 14 } \\
\mathrm{D}_{\mathrm{KL}}\left(p\left(\phi, S, z_{i}\right) \| p\left(\phi, S^{\backslash i}, z_{i}\right)\right) & \leq \epsilon
\end{aligned}
$$

## A. 4 Lemma A. 1

Lemma A. 1 (Upper bound on Input Loss Curvature). Let $\mathcal{A}$ be a randomized algorithm which is $\epsilon$-differentially private and the assumptions of error stability 2, generalization 3 and uniform model bias 4 hold. Further, assume $0 \leq \ell \leq L$. Then for two adjacent datasets $S, S^{i} \sim \mathcal{D}$ with a probability at least $1-\delta$ we have:

$$
\begin{gather*}
\mathbb{E}_{\phi}\left[\operatorname{Curv}_{\phi}\left(z_{i}, S^{\backslash i}\right)\right] \leq L\left(m\left(1-e^{-\epsilon}\right)+1\right)+c_{1}  \tag{15}\\
c_{1}=(4 m-1) \gamma+2(m-1) \Delta+\frac{\rho}{6} \mathbb{E}\left[\|\alpha\|^{3}\right] \tag{16}
\end{gather*}
$$

Proof of Lemma A.1 For this proof, we use results from Nesterov and Polyak [2006], we restate it here for convenience
Lemma A.2. If Lipschitz assumption 5 on the Hessian of $\ell$ holds from Nesterov and Polyak [2006] we have

$$
\begin{equation*}
\left|\ell\left(h, z_{1}\right)-\ell\left(h, z_{2}\right)-\left\langle\nabla \ell\left(h, z_{2}\right), z_{1}-z_{2}\right\rangle-\left\langle\nabla^{2} \ell\left(h, z_{2}\right)\left(z_{1}-z_{2}\right), z_{1}-z_{2}\right\rangle\right| \leq \frac{\rho}{6}\left|z_{1}-z_{2}\right|^{3} \tag{17}
\end{equation*}
$$
...
$$
\begin{equation*}
-\frac{\rho}{6}\left|z_{1}-z_{2}\right|^{3}+\ell\left(h, z_{2}\right)+\left\langle\nabla \ell\left(h, z_{2}\right), z_{1}-z_{2}\right\rangle+\left\langle\nabla^{2} \ell\left(h, z_{2}\right)\left(z_{1}-z_{2}\right), z_{1}-z_{2}\right\rangle \leq \ell\left(h, z_{1}\right) \tag{18}
\end{equation*}
$$

Consider $z_{j} \in S$ such that $z_{i}=z_{j}+\alpha$ for some $j \neq i$ where $\alpha \in B_{p}(v)$ such that $\mathbb{E}[\alpha]=0$ and $\mathbb{E}\left[\alpha^{T} \alpha\right]=1$. Using the lower bound in Lemma A. 2 from Ravikumar et al. [2024] with $z_{1}=z_{i}, z_{2}=z_{j}$ we get

$$
\begin{equation*}
\mathbb{E}_{\phi}\left[\ell\left(h_{S \backslash i}^{\phi}, z_{j}\right)\right]-\mathbb{E}_{\phi}\left[\ell\left(h_{S}^{\phi}, z_{i}\right)\right] \leq m \beta+(4 m-1) \gamma+2(m-1) \Delta \tag{19}
\end{equation*}
$$
...
$$
\begin{align*}
-\frac{\rho}{6}\|\alpha\|^{3} & +\mathbb{E}_{\phi}\left[\ell\left(h_{S \backslash i}^{\phi}, z_{i}\right)\right]+\mathbb{E}_{\phi}\left[\left\langle\nabla \ell\left(h_{S \backslash i}^{\phi}, z_{i}\right), \alpha\right\rangle\right] \\
& +\mathbb{E}_{\phi}\left[\left\langle\nabla^{2} \ell\left(h_{S \backslash i}^{\phi}, z_{i}\right) \alpha, \alpha\right\rangle\right]-\mathbb{E}_{\phi}\left[\ell\left(h_{S}^{\phi}, z_{i}\right)\right] \leq m \beta+(4 m-1) \gamma+2(m-1) \Delta \tag{20}
\end{align*}
$$
...
$$
\begin{aligned}
\mathbb{E}_{\phi}\left[\ell\left(h_{S \backslash i}^{\phi}, z_{i}\right)\right]+\mathbb{E}_{\alpha, \phi}\left[\left\langle\nabla \ell\left(h_{S \backslash i}^{\phi}, z_{i}\right), \alpha\right\rangle\right] & +\mathbb{E}_{\alpha, \phi}\left[\left\langle\nabla^{2} \ell\left(h_{S \backslash i}^{\phi}, z_{i}\right) \alpha, \alpha\right\rangle\right] \\
& -\mathbb{E}_{\phi}\left[\ell\left(h_{S}^{\phi}, z_{i}\right)\right] \leq m \beta+(4 m-1) \gamma+2(m-1) \Delta+\frac{\rho}{6}\|\alpha\|^{3}
\end{aligned}
$$
...
$$
\begin{aligned}
& \mathbb{E}_{\phi} {\left[\ell\left(h_{S \backslash i}^{\phi}, z_{i}\right)\right]+\mathbb{E}_{\phi, \alpha}\left[\left\langle\nabla^{2} \ell\left(h_{S \backslash i}^{\phi}, z_{i}\right) \alpha, \alpha\right\rangle\right]-\mathbb{E}_{\phi}\left[\ell\left(h_{S}^{\phi}, z_{i}\right)\right] \leq m \beta+(4 m-1) \gamma+2(m-1) \Delta+\frac{\rho}{6}\|\alpha\|^{3} } \\
& \mathbb{E}_{\phi}\left[\ell\left(h_{S \backslash i}^{\phi}, z_{i}\right)\right]+\mathbb{E}_{\phi}\left[\operatorname{tr}\left(\nabla^{2} \ell\left(h_{S \backslash i}^{\phi}, z_{i}\right)\right)\right]-\mathbb{E}_{\phi}\left[\ell\left(h_{S}^{\phi}, z_{i}\right)\right] \leq m \beta+(4 m-1) \gamma+2(m-1) \Delta+\frac{\rho}{6}\|\alpha\|^{3} \\
& \mathbb{E}_{\phi}\left[\operatorname{tr}\left(\nabla^{2} \ell\left(h_{S \backslash i}^{\phi}, z_{i}\right)\right)\right] \leq m \beta+(4 m-1) \gamma+2(m-1) \Delta+\frac{\rho}{6} \mathbb{E}\left[\|\alpha\|^{3}\right]+\mathbb{E}_{\phi}\left[\ell\left(h_{S}^{\phi}, z_{i}\right)\right]-\mathbb{E}_{\phi}\left[\ell\left(h_{S \backslash i}^{\phi}, z_{j}\right)\right] \\
& \mathbb{E}_{\phi}\left[\operatorname{tr}\left(\nabla^{2} \ell\left(h_{S}^{\phi}, z_{i}\right)\right)\right] \leq m \beta+(4 m-1) \gamma+2(m-1) \Delta+\frac{\rho}{6} \mathbb{E}\left[\|\alpha\|^{3}\right]+L
\end{aligned}
$$

From Ravikumar et al. 2024 Lemma 5.2 we have $\beta \leq L\left(1-e^{-\epsilon}\right)$. Thus we have the upper bound:

$$
\begin{align*}
\mathbb{E}_{\phi}\left[\operatorname{Curv}_{\phi}\left(z, S^{\backslash i}\right)\right] & \leq L\left(m\left(1-e^{-\epsilon}\right)+1\right)+c_{1}  \tag{21}\\
c_{1}=(4 m-1) \gamma & +2(m-1) \Delta+\frac{\rho}{6} \mathbb{E}\left[\|\alpha\|^{3}\right] \tag{22}
\end{align*}
$$

## A. 5 Proof of Theorem 4.2

This proof uses results from Lemma A.1 provided above. For the proof of Theorem 4.2 let $p_{c}\left(\phi, S, z_{i}\right)$ be the curvature probability mass function, then using the Gaussian model discussed in Section 4 we model it as a Gaussian. Thus, we can write

$$
\begin{align*}
p_{c}\left(\phi, S, z_{i}\right) & =\frac{1}{\sigma_{S} \sqrt{2 \pi}} e^{-\frac{\left(\phi-\mu_{S}\right)^{2}}{2 \sigma_{S}^{2}}}  \tag{23}\\
p_{c}\left(\phi, S^{\backslash i}, z_{i}\right) & =\frac{1}{\sigma_{S \backslash i} \sqrt{2 \pi}} e^{-\frac{\left(\phi-\mu_{S \backslash i}\right)^{2}}{2 \sigma_{S \backslash i}^{2}}} \tag{24}
\end{align*}
$$

Now consider the $\mathrm{D}_{\mathrm{KL}}\left(p_{c}\left(\phi, S, z_{i}\right) \| p_{c}\left(\phi, S^{\backslash i}, z_{i}\right)\right)$

$$
\begin{align*}
\mathrm{D}_{\mathrm{KL}}\left(p_{c}\left(\phi, S, z_{i}\right) \| p_{c}\left(\phi, S^{\backslash i}, z_{i}\right)\right) & =\mathbb{E}_{\phi}\left[\log \left(\frac{p_{c}\left(\phi, S, z_{i}\right)}{\left.p_{c}\left(\phi, S \backslash i, z_{i}\right)\right)}\right)\right]  \tag{25}\\
& =\mathbb{E}_{\phi}\left[-\frac{\left(\phi-\mu_{S}\right)^{2}}{2 \sigma^{2}}+\frac{\left(\phi-\mu_{S \backslash i}\right)^{2}}{2 \sigma^{2}}\right] \quad \text { Assume } \sigma=\sigma_{S \backslash i}=\sigma_{S}  \tag{26}\\
& =\mathbb{E}_{\phi}\left[\frac{-\phi^{2}-\mu_{S}^{2}+2 \phi \mu_{S}+\phi^{2}+\mu_{S \backslash i}^{2}-2 \phi \mu_{S \backslash i}}{2 \sigma^{2}}\right]  \tag{27}\\
& =\mathbb{E}_{\phi}\left[\frac{\mu_{S \backslash i}^{2}-\mu_{S}^{2}}{2 \sigma^{2}}\right]  \tag{28}\\
& =\frac{\mu_{S \backslash i}^{2}-\mu_{S}^{2}}{2 \sigma^{2}} \tag{29}
\end{align*}
$$

To upper bound Equation 29, we need an upper bound on $\mu_{S \backslash i}$ and a lower bound on $\mu_{S}$. The lower bound on $\mu_{S}^{2}=0$
For ease of notation let's define $\beta_{\text {max }}:=L\left(m\left(1-e^{-\epsilon}\right)+1\right)$. Notice that the upper bound of per sample $\mu_{S \backslash i}=\mathbb{E}_{\phi}\left[\operatorname{Curv}_{\phi}\left(z, S^{\backslash i}\right)\right]$. Using the result from Lemma A. 1 in Equation 29 we have

$$
\begin{align*}
\mathrm{D}_{\mathrm{KL}}\left(p_{c}\left(\phi, S, z_{i}\right) \| p_{c}\left(\phi, S^{\backslash i}, z_{i}\right)\right) & \leq \frac{\left(\beta_{\max }+c_{1}\right)^{2}-0^{2}}{2 \sigma^{2}}  \tag{30}\\
& \leq \frac{\left(\beta_{\max }+c_{1}\right)^{2}}{2 \sigma^{2}}  \tag{31}\\
& \leq \frac{\left[L\left(m\left(1-e^{-\epsilon}\right)+1\right)+c_{1}\right]^{2}}{2 \sigma^{2}} \tag{32}
\end{align*}
$$
...
$$
\begin{gather*}
\mathrm{D}_{\mathrm{KL}}\left(p_{c}\left(\phi, S, z_{i}\right) \| p_{c}\left(\phi, S^{\backslash i}, z_{i}\right)\right)>\mathrm{D}_{\mathrm{KL}}\left(p\left(\phi, S, z_{i}\right) \| p\left(\phi, S^{\backslash i}, z_{i}\right)\right)  \tag{33}\\
\frac{\left[L\left(m\left(1-e^{-\epsilon}\right)+1\right)+c_{1}\right]^{2}}{2 \sigma^{2}}>\epsilon \\
\frac{\left[L m\left(1-e^{-\epsilon}\right)+c\right]^{2}}{2 \sigma^{2}}>\epsilon \quad ; c=c_{1}+L \\
L^{2}\left(1-e^{-\epsilon}\right)^{2} m^{2}+2 L c\left(1-e^{-\epsilon}\right) m+c^{2}-2 \sigma^{2} \epsilon>0
\end{gather*}
$$
...
$$
\begin{aligned}
& =\frac{-2 L c\left(1-e^{-\epsilon}\right) \pm \sqrt{4 L^{2} c^{2}\left(1-e^{-\epsilon}\right)^{2}-4 L^{2}\left(1-e^{-\epsilon}\right)^{2}\left(c^{2}-2 \sigma^{2} \epsilon\right)}}{2 L^{2}\left(1-e^{-\epsilon}\right)^{2}} \\
& =\frac{-2 L c \pm \sqrt{4 L^{2} c^{2}-4 L^{2}\left(c^{2}-2 \sigma^{2} \epsilon\right)}}{2 L^{2}\left(1-e^{-\epsilon}\right)} \\
& =\frac{-L c \pm \sqrt{L^{2} c^{2}-L^{2}\left(c^{2}-2 \sigma^{2} \epsilon\right)}}{L^{2}\left(1-e^{-\epsilon}\right)} \\
& =\frac{-c \pm \sqrt{c^{2}-\left(c^{2}-2 \sigma^{2} \epsilon\right)}}{L\left(1-e^{-\epsilon}\right)} \\
& =\frac{-c \pm \sqrt{2 \sigma^{2} \epsilon}}{L\left(1-e^{-\epsilon}\right)}
\end{aligned}
$$

Assuming both roots are real, the larger root is $\frac{\sqrt{2 \sigma^{2} \epsilon}-c}{L\left(1-e^{-\epsilon}\right)}$ Thus when $m>\frac{\sqrt{2 \sigma^{2} \epsilon}-c}{L\left(1-e^{-\epsilon}\right)}$ Equation 33 is satisfied. $\square$

## A. 7 Input Augmentations Ablation
...
| Method | ImageNet |  |  |  |
| :--- | :---: | :---: | :---: | :---: |
|  | 1 Augmentation |  | 2 Augmentations |  |
|  | Bal. Acc. | AUROC | Bal. Acc. | AUROC |
| Curv ZO NLL (Ours) | $68.09 \pm 0.07$ | $75.84 \pm 0.08$ | $69.16 \pm 0.08$ | $77.45 \pm 0.09$ |
| Curv ZO LR (Ours) | $67.59 \pm 0.05$ | $70.62 \pm 0.04$ | $68.76 \pm 0.04$ | $72.28 \pm 0.04$ |
| Carlini et al. | 2022 | $65.11 \pm 0.03$ | $71.96 \pm 0.04$ | $66.14 \pm 0.01$ |
...
|  | 1 Augmentation |  | 2 Augmentations |  |
|  | Bal. Acc. | AUROC | Bal. Acc. | AUROC |
| Curv ZO NLL (Ours) | $83.97 \pm 0.23$ | $92.72 \pm 0.23$ | $84.47 \pm 0.21$ | $93.49 \pm 0.18$ |
| Curv ZO LR (Ours) | $77.88 \pm 0.21$ | $88.10 \pm 0.12$ | $80.48 \pm 0.10$ | $90.15 \pm 0.04$ |
| Carlini et al. | 2022 | $80.73 \pm 0.16$ | $87.69 \pm 0.18$ | $81.55 \pm 0.13$ |
...
|  | 1 Augmentation |  | 2 Augmentations |  |
|  | Bal. Acc. | AUROC | Bal. Acc. | AUROC |
| Curv ZO NLL (Ours) | $61.03 \pm 0.84$ | $67.56 \pm 1.23$ | $61.92 \pm 0.87$ | $68.82 \pm 1.30$ |
| Curv ZO LR (Ours) | $54.54 \pm 0.19$ | $58.06 \pm 0.41$ | $55.00 \pm 0.17$ | $58.89 \pm 0.38$ |
| Carlini et al. \|\|2022] | $56.96 \pm 0.27$ | $60.05 \pm 0.34$ | $58.23 \pm 0.29$ | $61.73 \pm 0.32$ |
...
| Method | 1 Augmentation |  | 2 Augmentations |  |
| :--- | :--- | :--- | :--- | :--- |
|  | TPR @ 0.1\% FPR | TPR @ 0.01\% FPR | TPR @ 0.1\% FPR | TPR @ 0.01\% FPR |
| Curv ZO LR (Ours) | $\mathbf{2 1 . 0 7} \pm \mathbf{0 . 8 0}$ | $\mathbf{1 5 . 7 4} \pm \mathbf{3 . 1 2}$ | $\mathbf{2 3 . 9 2} \pm \mathbf{0 . 9 2}$ | $\mathbf{1 7 . 1 7} \pm \mathbf{1 . 8 7}$ |
| Curv ZO NLL (Ours) | $6.42 \pm 0.62$ | $0.05 \pm 0.04$ | $8.29 \pm 1.52$ | $0.10 \pm 0.14$ |
| Carlini et al. \|2022 | $15.52 \pm 0.54$ | $5.02 \pm 1.70$ | $15.80 \pm 0.49$ | $6.56 \pm 1.13$ |
| Sablayrolles et al. [2019] | $10.50 \pm 0.59$ | $4.30 \pm 0.53$ | $10.50 \pm 0.59$ | $4.30 \pm 0.53$ |
| Watson et al. [2022] | $6.62 \pm 0.23$ | $2.95 \pm 0.18$ | $6.62 \pm 0.23$ | $2.95 \pm 0.18$ |
| Ye et al. \|2022 | $6.63 \pm 0.52$ | $2.86 \pm 0.42$ | $6.63 \pm 0.52$ | $2.86 \pm 0.42$ |
| Song and Mittal \|2021 | $1.21 \pm 0.37$ | $1.21 \pm 0.37$ | $1.21 \pm 0.37$ | $1.21 \pm 0.37$ |
| Yeom et al. \|2018 | $0.07 \pm 0.01$ | $0.01 \pm 0.00$ | $0.07 \pm 0.01$ | $0.01 \pm 0.00$ |

Table 5: Comparison of TPR @ 0.1\% FPR and TPR @ 0.01\% FPR for CIFAR100 dataset with 1 and 2 augmentations. Results reported are the mean $\pm$ std obtained over 3 seeds and using 64 shadow models.

![](https://cdn.mathpix.com/cropped/2025_11_08_81705b7b7843ea88add3g-18.jpg?height=543&width=1386&top_left_y=1035&top_left_x=368)
Figure 8: TPR at low FPR visualizing MIA performance as a function of the size of the train set, which is randomly sampled.

## A. 9 ZO Estimation Performance
...
| Dataset | Method | AUROC | Balanced Accuracy |
| :--- | :--- | :--- | :--- |
| CIFAR100 | Curv ZO NLL | $93.49 \pm 0.18$ | $84.47 \pm 0.21$ |
|  | Curv NLL | $95.01 \pm 0.19$ | $85.66 \pm 0.30$ |
|  | Curv ZO LR | $90.15 \pm 0.04$ | $80.48 \pm 0.10$ |
|  | Curv LR | $92.87 \pm 0.02$ | $83.75 \pm 0.16$ |
| CIFAR10 | Curv ZO NLL | $68.82 \pm 1.30$ | $61.92 \pm 0.87$ |
|  | Curv NLL | $74.20 \pm 1.42$ | $64.96 \pm 1.05$ |
|  | Curv ZO LR | $58.89 \pm 0.38$ | $55.00 \pm 0.17$ |
|  | Curv LR | $68.53 \pm 0.27$ | $61.82 \pm 0.11$ |
...
## A. 10 Performance Across Architectures
...
| Arch. | Method |  | AUROC | Bal. Acc. | TPR @ 1\% FPR |
| :--- | :--- | :--- | :--- | :--- | :--- |
| Inception | Curv ZO NLL |  | $87.77 \pm 0.61$ | $79.04 \pm 0.65$ | $15.33 \pm 1.38$ |
|  | Curv ZO LR |  | $81.94 \pm 0.79$ | $73.46 \pm 0.88$ | $19.46 \pm 0.81$ |
|  | Carlini et al. | 2022 | $83.77 \pm 0.63$ | $76.70 \pm 0.66$ | $16.66 \pm 0.82$ |
| VGG11 | Curv ZO NLL |  | $73.68 \pm 2.99$ | $66.53 \pm 3.35$ | $6.19 \pm 1.15$ |
|  | Curv ZO LR |  | $68.29 \pm 1.38$ | $64.30 \pm 1.13$ | $8.14 \pm 1.85$ |
|  | Carlini et al. | 2022 | $62.85 \pm 0.71$ | $61.08 \pm 0.87$ | $2.07 \pm 0.19$ |
| VGG11BN | Curv ZO NLL |  | $87.76 \pm 0.17$ | $79.12 \pm 0.11$ | $15.56 \pm 1.34$ |
|  | Curv ZO LR |  | $76.17 \pm 0.65$ | $70.98 \pm 0.26$ | $22.80 \pm 0.40$ |
|  | Carlini et al. | 2022 | $70.57 \pm 0.39$ | $67.13 \pm 0.43$ | $3.22 \pm 0.34$ |
| ViT-B16 | Curv ZO NLL |  | $58.10 \pm 0.20$ | $55.40 \pm 0.18$ | $1.60 \pm 0.01$ |
|  | Curv ZO LR |  | $53.98 \pm 0.30$ | $52.55 \pm 0.18$ | $1.91 \pm 0.25$ |
|  | Carlini et al. | 2022 | $54.59 \pm 0.05$ | $53.22 \pm 0.10$ | $1.44 \pm 0.14$ |
...
Compute Resources. All of the experiments were performed on a heterogeneous compute cluster consisting of 9 1080Ti's, 6 2080Ti's and 4 A40 NVIDIA GPUs, with a total of 100 CPU cores and a combined 1.2 TB of main system memory. However, the results can be replicated with a single GPU with 11 GB of VRAM.

Hyperparameters. Our code uses 2 hyper parameters for zero-order input loss curvature estimation. The $n_{\text {iter }}$ and $h$ in Algorithm 1. We used $n_{\text {iter }}=10$ and $h=0.001$. To improve reproducibility, we have provided the code in the supplementary material.

## A. 14 Licenses for Assets Used
...
Answer: [Yes]
Justification: We discuss the limitations of the work in Appendix A.12 where we note the increased computation requirement imposed by zero-order curvature calculation and other limitations of the proposed methods.
...
Answer: [Yes]
Justification: All the assumptions are stated in the Theorems and discussed prior to their utilization in the main paper. The complete proof has also been provided in the Appendix of the paper, specifically Appendix A.3 A.5 and A.6.
Guidelines:
...
Answer: [Yes]
Justification: We include all the training details for the models in Appendix A.13 We also provide the pseudo-code for the proposed technique in Appendix A. 2 along with all the hyper parameter details (see Appendix A.13). Further, we also provide the code in the supplementary material.
...
Answer: [Yes]
Justification: The datasets used are publicly available and are the standard datasets used for vision tasks. Our code is included in the supplementary material and will be released publicly after the conference deadline. The code replicates the results of our experiments.
Guidelines:
...
Answer: [Yes]
Justification: The training details are reported in Section 6.1 of the paper and more details are provided in Appendix A.13
Guidelines:
...
Answer: [Yes]
Justification: All reported results are obtained by averaging over 3 seeds and reported with mean and standard deviation values.
...
Answer: [Yes]
Justification: For the experiments, we used a heterogeneous cluster whose details are mentioned in Appendix A.13.
Guidelines:
...
Answer: [Yes]
Justification: The paper conforms to the code of ethics, we discus potential impacts of this research paper in Appendix A.11.
Guidelines:
...
Answer: [Yes]
Justification: We provide a detailed discussion about the broader impact of the research presented in the paper in Appendix A.11.
Guidelines:
...
Answer: [NA]
Justification: We do not release data or models thus this paper does not pose such risks.
Guidelines:
...
Answer: [Yes]
Justification: We provide the detailed list of assets used and the terms under which they were licensed in Appendix A.14 We have respected the licensing terms of the original owners.
Guidelines:
...
Answer: [Yes]
Justification: We release the code for reproducing the results under the MIT license and details of its usage are documented in Appendix A. 13 and the supplementary material README file.
Guidelines:
...
[^0]:    ${ }^{1}$ Code available at https://github.com/DeepakTatachar/Curvature-Clues
    ${ }^{2}$ Project website with models and other assets is available at https://engineering.purdue.edu/NRL/ projects/curvature-clues

#!/bin/bash
#SBATCH --job-name=upgd_input_aware_curv_0.01_seed_2_centered_linear_with_utility_norms_upgd_histograms
...
#SBATCH --output=/scratch/gautschi/shin283/upgd/logs/%j_upgd_input_aware_curv_0.01_seed_2_centered_linear_with_utility_norms_upgd_histograms.out
#SBATCH --error=/scratch/gautschi/shin283/upgd/logs/%j_upgd_input_aware_curv_0.01_seed_2_centered_linear_with_utility_norms_upgd.err
...
export WANDB_RUN_NAME="${SLURM_JOB_ID}_upgd_input_aware_curv_0.01_seed_2_centered_linear_with_utility_norms_histograms"
...
echo "End time: $(date)"
echo "JSON results saved to: logs/input_permuted_mnist_stats/input_aware_upgd_fo_global/fully_connected_relu_with_hooks/lr_0.01_sigma_0.1_curvature_threshold_0.01_lambda_max_2.0_hutchinson_samples_5_n_samples_1000000_disable_regularization_True_disable_gating_False_map_${UPGD_LAMBDA_MAPPING}_gate_${UPGD_GATING_STRATEGY}_a_${UPGD_OPTION_C_A}_b_${UPGD_OPTION_C_B}_ming_${UPGD_MIN_GATING}_n5_tau_0.01/0.json"
echo "WandB Run: ${WANDB_RUN_NAME}"

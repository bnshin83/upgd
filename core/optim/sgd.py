...
class SGD(torch.optim.Optimizer):
    ...
    def step(self):
        # First pass: compute utility and track global max
        ...
        for group in self.param_groups:
            for name, p in zip(group["names"], group["params"]):
                ...
                # SGD update with optional noise
                noise = torch.randn_like(p.grad) * group["sigma"] if group["sigma"] > 0 else 0
                # SGD: θ ← θ - η·∇L - η·σ·ε - η·λ·θ, where ε ~ N(0,I)
                # Difference from shrink_and_perturb.py: They use θ ← (1 - η·λ)θ - η(∇L + σ·ε)
                # which is mathematically equivalent but groups the decay term differently 
                # upgd: (θ ← (1 - η·λ)θ - η·[∇L + σ·ε·(1 - u)])
                # sgd: (θ ← θ - η·∇L - η·σ·ε - η·λ·θ)

#!/bin/bash
#SBATCH --job-name=incr_cifar_upgd_hidden_seeds
...
#SBATCH --array=0-4%3
#SBATCH --output=/scratch/gilbreth/shin283/upgd/logs/%A_%a_incr_cifar_upgd_hidden.out
#SBATCH --error=/scratch/gilbreth/shin283/upgd/logs/%A_%a_incr_cifar_upgd_hidden.err
...
for i in 0 1; do
    ...
    if [ $SEED_IDX -lt $NUM_SEEDS ]; then
        ...
        cat > $CONFIG_FILE <<EOF
{
  "_model_description_": "UPGD optimizer - ${GATING_MODE} gating (all layers except final FC)",
  "data_path": "",
  "results_dir": "",

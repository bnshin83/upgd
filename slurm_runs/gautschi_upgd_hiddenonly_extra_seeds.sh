#!/bin/bash
#SBATCH --job-name=upgd_hidonly_seeds
...
#SBATCH --array=0-19%8
#SBATCH --output=/scratch/gautschi/shin283/upgd/logs/%A_%a_upgd_hidonly.out
#SBATCH --error=/scratch/gautschi/shin283/upgd/logs/%A_%a_upgd_hidonly.err

# UPGD Hidden-Only extra seeds: 4 datasets x 5 seeds = 20 jobs
...
export WANDB_RUN_NAME="${SLURM_JOB_ID}_upgd_hidonly_${TASK_NAME}_seed_${SEED}"

echo "========================================="
echo "UPGD Hidden-Only: ${TASK_NAME} seed=${SEED}"
...
echo "UPGD Hidden-Only ${TASK_NAME} seed=${SEED} completed at $(date)"
